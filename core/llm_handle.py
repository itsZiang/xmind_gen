...
def initialize_workflow_manager():
    ...
    workflow_manager.initialize(BASE_URL, PROVIDER, API_KEY)

def generate_xmindmark_with_workflow(text: str, user_requirements: str) -> dict:
    """Tạo XMindMark sử dụng LangGraph workflow"""
    if not workflow_manager.workflow:
        initialize_workflow_manager()
    
    return workflow_manager.generate_mindmap_with_workflow(text, user_requirements)

def edit_xmindmark_with_workflow(current_content: str, edit_request: str) -> dict:
    """Chỉnh sửa XMindMark sử dụng LangGraph workflow"""
    if not workflow_manager.workflow:
        initialize_workflow_manager()
    
    return workflow_manager.edit_mindmap_with_workflow(current_content, edit_request)

# Giữ lại các hàm cũ để backward compatibility
def generate_xmindmark(text: str, user_requirements: str) -> str:
    """Legacy function - sử dụng workflow bên trong"""
    result = generate_xmindmark_with_workflow(text, user_requirements)
    return result.get("xmindmark", "")

def edit_xmindmark_with_llm(current_content: str, edit_request: str) -> str:
    """Legacy function - sử dụng workflow bên trong"""
    result = edit_xmindmark_with_workflow(current_content, edit_request)
    return result.get("edited_xmindmark", current_content)
