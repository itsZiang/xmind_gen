...
def create_xmindmark_prompt(text: str, user_requirements: str, conversation_history: Optional[List[Dict[str, str]]] = None) -> str:
    history_context = ""
    if conversation_history:
        history_context = "\n".join([f"{item['role']}: {item['content']}" for item in conversation_history[-5:]])
        history_context = f"**LỊCH SỬ HỘI THOẠI:**\n{history_context}\n\n"
...
- Trả về CHỈ nội dung định dạng xmindmark, không bao gồm giải thích hoặc ký tự ngoài định dạng như ```, ```json
Ví dụ:
ông nội
- cha1
    - con1
...
def create_global_title_prompt(text: str, user_requirements: str, conversation_history: Optional[List[Dict[str, str]]] = None) -> str:
    history_context = ""
    if conversation_history:
        history_context = "\n".join([f"{item['role']}: {item['content']}" for item in conversation_history[-5:]])
        history_context = f"**LỊCH SỬ HỘI THOẠI:**\n{history_context}\n\n"
...
def create_split_text_prompt(text: str, user_requirements: str, conversation_history: Optional[List[Dict[str, str]]] = None) -> str:
    history_context = ""
    if conversation_history:
        history_context = "\n".join([f"{item['role']}: {item['content']}" for item in conversation_history[-5:]])
        history_context = f"**LỊCH SỬ HỘI THOẠI:**\n{history_context}\n\n"
...
def create_edit_prompt(current_xmindmark: str, edit_request: str, conversation_history: Optional[List[Dict[str, str]]] = None) -> str:
    history_context = ""
    if conversation_history:
        history_context = "\n".join([f"{item['role']}: {item['content']}" for item in conversation_history[-5:]])
        history_context = f"**LỊCH SỬ HỘI THOẠI:**\n{history_context}\n\n"
...
- TUYỆT ĐỐI không dùng các ký tự thừa như ```, ```json
VÍ DỤ FORMAT ĐÚNG:
...
def create_merge_xmindmark_prompt(chunks_text: str, global_title: str, user_requirements: str, conversation_history: Optional[List[Dict[str, str]]] = None) -> str:
    history_context = ""
    if conversation_history:
        history_context = "\n".join([f"{item['role']}: {item['content']}" for item in conversation_history[-5:]])
        history_context = f"**LỊCH SỬ HỘI THOẠI:**\n{history_context}\n\n"
...
def create_xmindmark_no_docs_prompt(user_requirements: str, conversation_history: Optional[List[Dict[str, str]]] = None) -> str:
    history_context = ""
    if conversation_history:
        history_context = "\n".join([f"{item['role']}: {item['content']}" for item in conversation_history[-5:]])
        history_context = f"**LỊCH SỬ HỘI THOẠI:**\n{history_context}\n\n"
...
def create_xmindmark_with_search_prompt(search_context: str, user_requirements: str, conversation_history: Optional[List[Dict[str, str]]] = None) -> str:
    history_context = ""
    if conversation_history:
        history_context = "\n".join([f"{item['role']}: {item['content']}" for item in conversation_history[-5:]])
        history_context = f"**LỊCH SỬ HỘI THOẠI:**\n{history_context}\n\n"
    ...
    prompt = f"""
{history_context}
Bạn là chuyên gia tạo sơ đồ tư duy. Nhiệm vụ của bạn là tạo một sơ đồ tư duy dựa trên yêu cầu của người dùng và thông tin bổ sung từ tìm kiếm. Thông tin bổ sung được cung cấp từ Tavily Search, bao gồm nội dung từ 3 kết quả đầu tiên. Hãy kết hợp thông tin này một cách logic, tránh lặp lại và bám sát yêu cầu của người dùng.
**THÔNG TIN BỔ SUNG:**
{search_context}
**YÊU CẦU CỦA NGƯỜI DÙNG:**
{user_requirements}
